...
if __name__ == "__main__":
    ...
    print(image.shape)
    batch_size = 7
    image_data = [image, image, image, image, image, image, image]
    image_data = np.array(image_data)
    image_data = np.squeeze(image_data)
    triton_client = tritonhttpclient.InferenceServerClient("127.0.0.1:8000", verbose=False)
    inputs = []
    inputs.append(tritonhttpclient.InferInput("000_net", [batch_size, image.shape[1], image.shape[2], image.shape[3]], "FP32"))
    ...
    for i in range(batch_size):
        trtis_output = [np.expand_dims(output1[i], 0), np.expand_dims(output2[i], 0), np.expand_dims(output3[i], 0)]
        ...
        output_image_path = 'output_dog_{}.png'.format(i)
