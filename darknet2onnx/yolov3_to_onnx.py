# python3
"""
Created by HenryMa on 2020/4/27
"""
...
class DarkNetParser(object):
    ...
    def __init__(self, supported_layers):
        ...
        :param supported_layers: a string list of supported layers in DarkNet naming convention,
    ...
    def parse_cfg_file(self, cfg_file_path):
        ...
        :param cfg_file_path: path to the yolov3.cfg file as string
        :return:
        """
        with open(cfg_file_path, 'rb') as cfg_file:
            remainder = cfg_file.read().decode('utf8')
    ...
    def _next_layer(self, remainder):
        ...
        :param remainder: a string with all raw text after the previously parsed layer
        :return:
        """
    ...
    def _parse_params(param_line):
        ...
        them in the required format for each parameter type, e.g. as a list,
        an int or a float.
        :param param_line: one parsed line within a layer block
        :return:
        """
...
class MajorNodeSpecs(object):
    ...
    def __init__(self, name, channels):
        ...
        :param name: name of the ONNX node
        :param channels: number of output channels of this node
        """
        self.name = name
...
class ConvParams(object):
    ...
    Additionally acts as a wrapper for generating safe name for all
    ...
    def __init__(self, node_name, batch_normalize, conv_weight_dims):
        ...
        :param node_name: base name of this YOLO convolutional layer
        :param batch_normalize: bool value if batch normalization is used
        :param conv_weight_dims: the dimensions of this layer's convolutional weights
        """
...
class UpsampleParams(object):
    ...
    def __init__(self, node_name, value):
        ...
        :param node_name: base name of this YOLO Upsample layer
        :param value: the value of the scale input to the Upsample layer as a numpy array
        """
        self.node_name = node_name
        self.value = value
...
class WeightLoader(object):
    ...
    def _create_param_tensors(self, conv_params, param_category, suffix):
        ...
        :param conv_params: a ConvParams object
        :param param_category: the category of params
        :param suffix: a string determining the sub-type of above param_category (e.g., 'weights' or 'bias')
        :return:
        """
    ...
    def _load_one_param_type(self, conv_params, param_category, suffix):
        """
        Deserialized the weights from a file stream in the DarkNet order.
        :param conv_params: a ConvParams object
        :param param_category: the category of parameters to be created ('bn' or 'conv')
        :param suffix: a string determining the sub-type of above param_category (e.g., 'weights' or 'bias')
        :return:
        """
...
class GraphBuilderONNX(object):
    """
    Class for creating an ONNX from a previously generated list of layer dictionaries.
    """
    def __init__(self, output_tensors):
        ...
        with their name as keys.
        :param output_tensors: the output tensors as an OrderedDict containing the keys' output dimensions
        """
    ...
    def build_onnx_graph(self, layer_configs, weights_file_path, verbose=True):
        ...
        :param layer_configs: an OrderedDict object with all parsed layers' configurations
        :param weights_file_path: location of the weights file
        :param verbose: toggles if the graph is printed after creation (default: True)
        :return:
        """
    ...
    def _make_onnx_node(self, layer_name, layer_dict):
        ...
        :param layer_name: the layer's name (also the corresponding key in layer_configs)
        :param layer_dict: a layer parameter dictionary (one element of layer_configs)
        :return:
        """
        ...
        if self.input_tensor is None:
            if layer_type == 'net':
                ...
                raise ValueError('The first node has to be of type "int".')
        else:
            node_creators = dict()
    ...
    def _make_input_tensor(self, layer_name, layer_dict):
        ...
        :param layer_name: the layer's name (also he corresponding key in layer_configs)
        :param layer_dict:
        :return:
        """
    ...
    def _get_previous_node_specs(self, target_index=-1):
        ...
        :param target_index: optional for jumping to a specific index (default: -1 for jumping to previous element
        :return:
        """
        previous_node = None
...
def main():
    ...
    # Create a DarkNetParser object, and use it to generate an OrderedDict with all
